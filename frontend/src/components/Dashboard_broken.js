                ...
                <div className="bg-white rounded-lg shadow-md border p-6 mb-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="text-center">
                      <div className={`text-lg font-semibold ${
                        result.tracking_info.status === 'Delivered' ? 'text-green-600' : 
                        result.tracking_info.status === 'In Transit' ? 'text-blue-600' : 
                        'text-orange-600'
                      }`}>
                    ...
                    <div className="text-center">
                      <div className="text-lg font-semibold text-gray-700">{result.tracking_info.service || 'Express'}</div>
                  ...
                  <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="text-gray-900">{result.tracking_info.origin || 'No disponible'}</div>
                    </div>
                    <div>
                      <div className="text-sm font-medium text-gray-500">Destino</div>
                      <div className="text-gray-900">{result.tracking_info.destination || 'No disponible'}</div>
                    </div>
                    <div>
                      <div className="text-sm font-medium text-gray-500">Fecha de Envío</div>
                      <div className="text-gray-900">{result.tracking_info.shipment_date || 'No disponible'}</div>
                    </div>
                    <div>
                      <div className="text-sm font-medium text-gray-500">Peso Total</div>
                      <div className="text-gray-900">{result.tracking_info.weight || 'No disponible'}</div>
                    </div>
                  </div>

                  {/* Información adicional de piezas */}
                  {result.tracking_info.pieces && result.tracking_info.pieces > 0 && (
                    <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                      <div className="text-sm font-medium text-gray-700">
                        <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                          {result.tracking_info.pieces} {result.tracking_info.pieces === 1 ? 'pieza' : 'piezas'}
                        </span>
                      </div>
                    </div>
                  )}
                        ...
                        <div key={pieceIndex} className="border rounded-lg p-4">
                          <div className="flex justify-between items-start mb-3">
                            <h6 className="font-medium text-gray-800">
                              Pieza {piece.piece_number} - {piece.license_plate}
                            </h6>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            <div>
                              ...
                              <span className="ml-1 font-medium">{piece.actual_weight} {piece.weight_unit}</span>
                            </div>
                            <div>
                              <span className="text-gray-500">Dimensiones:</span>
                              <span className="ml-1 font-medium">
                                {piece.actual_length} × {piece.actual_width} × {piece.actual_height}
                              </span>
                            </div>
                            <div>
                              <span className="text-gray-500">Peso Vol.:</span>
                              <span className="ml-1 font-medium">{piece.dim_weight} {piece.weight_unit}</span>
                            </div>
                            <div>
                              <span className="text-gray-500">Declarado:</span>
                              <span className="ml-1 font-medium">{piece.declared_weight} {piece.weight_unit}</span>
                            </div>
                          </div>
                        </div>
                      ))}
                ...
                <div className="bg-white rounded-lg shadow-md border p-6">
                  ...
                  {result.events && result.events.length > 0 ? (
                      ...
                      {result.events.map((event, index) => (
                        <div key={index} className="flex items-start space-x-4">
                          <div className={`flex-shrink-0 w-3 h-3 rounded-full mt-2 ${
                            event.code === 'OK' ? 'bg-green-500' : 
                            event.code === 'CC' ? 'bg-yellow-500' :
                            event.code === 'PU' ? 'bg-blue-500' :
                            event.code === 'AF' ? 'bg-purple-500' :
                            event.code === 'PL' ? 'bg-indigo-500' :
                            event.code === 'DF' ? 'bg-cyan-500' :
                            event.code === 'CR' ? 'bg-emerald-500' :
                            'bg-gray-400'
                          }`}></div>
                              ...
                              <div>
                                <div className="flex items-center space-x-2">
                                  ...
                                  <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium ${
                                    event.code === 'OK' ? 'bg-green-100 text-green-800' :
                                    event.code === 'CC' ? 'bg-yellow-100 text-yellow-800' :
                                    event.code === 'PU' ? 'bg-blue-100 text-blue-800' :
                                ...
                                {event.signatory && (
                                  <p className="text-sm text-green-600 mt-1">
                                    <span className="font-medium">Firmado por:</span> {event.signatory}
                                  </p>
                                )}
                              </div>
                    ...
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="mt-2">No hay eventos de seguimiento disponibles</p>
                    </div>
                  )}
                </div>

                {/* Resumen de entrega si está disponible */}
                {result.tracking_info.status === 'Delivered' && (
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="ml-3">
                        ...
                        <div className="mt-2 text-sm text-green-700">
                          <p>El envío ha sido entregado exitosamente</p>
                          {result.events && result.events.length > 0 && (
                            <p className="mt-1">
                              <span className="font-medium">Fecha:</span> {result.events[result.events.length - 1]?.date} {result.events[result.events.length - 1]?.time}
                            </p>
                          )}
                        </div>
                      </div>
